...
required for running service replicas/instances of service types specified in the service manifest. The configuration settings 
must be stored in Settings.xml in the config package folder. 

The service developer uses Service Fabric APIs to locate the package folders and read application overridable configuration settings.
The service developer can also register callbacks that are invoked when any of the configuration packages specified in the
...
configuration packages are upgraded. 